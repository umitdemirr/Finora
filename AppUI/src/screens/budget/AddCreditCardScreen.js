...
const AddCreditCardScreen = ({ navigation }) => {
    ...
    try {
      ...
      if (response.data.success && response.data.data) {
        ...
        if (response.data.data.length > 0) {
          setNewCard(prev => ({
            ...prev,
            bankId: response.data.data[0].accountId
          }));
        }
      }
    } catch (error) {
  ...
  const handleAddCard = async () => {
    try {
      if (!newCard.name || !newCard.number || !newCard.expiryDate || !newCard.cvv || !newCard.bankId || !newCard.limit) {
      ...
      const response = await api.post('/CreditCard/add', {
        ...newCard,
        userId: parseInt(userId),
        id: 0,
        avaliableLimit: parseFloat(newCard.limit), // Başlangıçta limit ile aynı
